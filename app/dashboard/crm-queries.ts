...
export async function getAccountDetails(id: string) {
  ...
  const [relatedContacts, relatedLeads, relatedDeals, openTasks, events] = await Promise.all([
    db.select({ id: contacts.id, fullName: contacts.fullName, email: contacts.email, title: contacts.title, isPrimary: contacts.isPrimary }).from(contacts).where(and(eq(contacts.accountId, id), eq(contacts.isArchived, false), ...getRecordVisibilityConditions(context, contacts.workspaceId, contacts.assignedOwnerUserId))).orderBy(asc(contacts.fullName)).limit(detailRelatedLimit),
    db.select({ id: leads.id, fullName: leads.fullName, status: leads.status, assignedOwnerUserId: leads.assignedOwnerUserId }).from(leads).where(and(eq(leads.accountId, id), eq(leads.isArchived, false), ...getRecordVisibilityConditions(context, leads.workspaceId, leads.assignedOwnerUserId))).orderBy(desc(leads.updatedAt)).limit(detailRelatedLimit),
    db.select({ id: deals.id, name: deals.name, stage: deals.stage, valueCents: deals.valueCents, currency: deals.currency, expectedCloseAt: deals.expectedCloseAt }).from(deals).where(and(eq(deals.accountId, id), ...getRecordVisibilityConditions(context, deals.workspaceId, deals.ownerUserId))).orderBy(desc(deals.updatedAt)).limit(detailRelatedLimit),
    db.select({ id: crmTasks.id, title: crmTasks.title, dueAt: crmTasks.dueAt, priority: crmTasks.priority }).from(crmTasks).innerJoin(deals, and(eq(crmTasks.dealId, deals.id), eq(crmTasks.workspaceId, deals.workspaceId))).where(and(eq(deals.accountId, id), eq(crmTasks.status, "pending"), ...getTaskVisibilityConditions(context, crmTasks.workspaceId, crmTasks.ownerUserId, crmTasks.userId), ...getRecordVisibilityConditions(context, deals.workspaceId, deals.ownerUserId))).orderBy(asc(crmTasks.dueAt)).limit(detailRelatedLimit),
...
export async function getContactDetails(id: string) {
  ...
  const [relatedLeads, relatedDeals, openTasks, events] = await Promise.all([
    db.select({ id: leads.id, fullName: leads.fullName, status: leads.status }).from(leads).where(and(eq(leads.primaryContactId, id), eq(leads.isArchived, false), ...getRecordVisibilityConditions(context, leads.workspaceId, leads.assignedOwnerUserId))).orderBy(desc(leads.updatedAt)).limit(detailRelatedLimit),
    db.select({ id: deals.id, name: deals.name, stage: deals.stage, valueCents: deals.valueCents, currency: deals.currency }).from(deals).where(and(eq(deals.contactId, id), ...getRecordVisibilityConditions(context, deals.workspaceId, deals.ownerUserId))).orderBy(desc(deals.updatedAt)).limit(detailRelatedLimit),
    db.select({ id: crmTasks.id, title: crmTasks.title, dueAt: crmTasks.dueAt, priority: crmTasks.priority }).from(crmTasks).where(and(eq(crmTasks.contactId, id), eq(crmTasks.status, "pending"), ...getTaskVisibilityConditions(context, crmTasks.workspaceId, crmTasks.ownerUserId, crmTasks.userId))).orderBy(asc(crmTasks.dueAt)).limit(detailRelatedLimit),
