...
export async function createDealAction(values: DealFormValues, idempotencyKey?: string): Promise<MutationResult> {
  ...
  let record: { id: string } | null;
  try {
    const result = await executeIdempotentMutation({ workspaceId: state.workspace.id, actorUserId: state.userId, action: "deal.create", idempotencyKey: idempotencyKey ?? requestId, request: parsed.data }, async (tx) => { if (!(await lockWorkspaceMember(tx, state.workspace.id, owner))) return { response: null }; if (!(await validateDealLinks(state.context, parsed.data, tx))) return { response: null }; const [created] = await tx.insert(deals).values({ workspaceId: state.workspace.id, userId: state.userId, ownerUserId: owner, leadId: parsed.data.leadId || null, accountId: parsed.data.accountId || null, contactId: parsed.data.contactId || null, name: parsed.data.name, stage: parsed.data.stage, valueCents: moneyToCents(parsed.data.value), currency: parsed.data.currency, probability: parsed.data.probability, expectedCloseAt: parsed.data.expectedCloseDate ? new Date(`${parsed.data.expectedCloseDate}T00:00:00Z`) : null, closedAt: ["won", "lost"].includes(parsed.data.stage) ? new Date() : null, lostReason: parsed.data.stage === "lost" ? parsed.data.lostReason : null }).onConflictDoNothing({ target: [deals.workspaceId, deals.leadId] }).returning({ id: deals.id }); if (!created) return { response: null }; await createCrmActivity({ client: tx, workspaceId: state.workspace.id, userId: state.userId, eventType: "deal_updated", message: `Deal created: ${parsed.data.name}`, dealId: created.id, leadId: parsed.data.leadId || null, accountId: parsed.data.accountId || null, contactId: parsed.data.contactId || null }); await writeAuditEvent({ tx, workspaceId: state.workspace.id, actor: { userId: state.userId, role: state.workspace.role }, action: "deal.created", entity: { type: "deal", id: created.id }, after: { name: parsed.data.name, stage: parsed.data.stage, ownerUserId: owner, leadId: parsed.data.leadId || null, accountId: parsed.data.accountId || null, contactId: parsed.data.contactId || null }, requestId }); return { response: created, resource: { type: "deal", id: created.id } }; });
    record = result.value;
  } catch (error) {
