...
async function activity(input: { workspaceId: string; userId: string; eventType: typeof activityEvents.$inferInsert.eventType; message: string; accountId?: string | null; contactId?: string | null; dealId?: string | null; leadId?: string | null; leadName?: string | null }) {
...
export async function createDealAction(values: DealFormValues): Promise<MutationResult> {
  ...
  const [record] = await db.insert(deals).values({ workspaceId: state.workspace.id, userId: state.userId, ownerUserId: owner, leadId: parsed.data.leadId || null, accountId: parsed.data.accountId || null, contactId: parsed.data.contactId || null, name: parsed.data.name, stage: parsed.data.stage, valueCents: moneyToCents(parsed.data.value), currency: parsed.data.currency, probability: parsed.data.probability, expectedCloseAt: parsed.data.expectedCloseDate ? new Date(`${parsed.data.expectedCloseDate}T00:00:00Z`) : null, closedAt: ["won", "lost"].includes(parsed.data.stage) ? new Date() : null, lostReason: parsed.data.stage === "lost" ? parsed.data.lostReason : null }).returning({ id: deals.id });
  await activity({ workspaceId: state.workspace.id, userId: state.userId, eventType: "deal_updated", message: `Deal created: ${parsed.data.name}`, dealId: record.id, leadId: parsed.data.leadId || null, accountId: parsed.data.accountId || null, contactId: parsed.data.contactId || null }); mutationPaths("/dashboard/deals", `/dashboard/deals/${record.id}`); return { success: true, id: record.id, message: "Deal created." };
}
