...
export default async function AccountsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <div className="space-y-5"><PageHeader eyebrow="Relationship management" title="Accounts" description="Keep companies, their contacts, and active opportunities connected in one place." action={<>{canImport ? <Button asChild variant="outline"><Link href="/dashboard/import"><Upload className="mr-2 h-4 w-4" />Import CSV</Link></Button> : null}{canCreate ? <Button asChild><Link href="/dashboard/accounts/new"><Plus className="mr-2 h-4 w-4" />Add account</Link></Button> : null}</>} />
